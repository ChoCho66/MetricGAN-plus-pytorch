...
class Trainer_finetune:
    ...
    def init_model_optim(self):
        ...
        layers = list(self.G.children())  # 獲取模型的子層
        for layer in layers[-num_layers_to_unfreeze:]:
            for param in layer.parameters():
                param.requires_grad = True

        # 重新設定 optimizer，只優化需要的層
        self.optimizer_g = torch.optim.Adam(filter(lambda p: p.requires_grad, self.G.parameters()), lr=self.args.lr)
    ...
    def evaluation(self, data_list, phase='valid'):
        ...
        if (phase == 'valid') or (phase == 'test'):    # Test path
            ...
            # print(f'Current PESQ Score: {test_scores["pesq"]}')
            # print(f'Best PESQ Score Before: {self.best_scores["pesq"]}')
